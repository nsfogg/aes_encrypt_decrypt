/**
*/

#include "aes.h"
#include "field.h"
#include <stdbool.h>
#include <stdlib.h>
#include <stdio.h>

/**
   Return the sBox substitution value for a given byte value.
   @param v byte input value.
   @return substitution for the given byte.
*/
static byte substBox( byte v )
{
  // Forward-direction replacement map for the sBox.
  static const byte rule[] =
    {  };

  // Return the replaceent for v based on the map.
  return rule[ v ];
}

/**
   Return the inverse sBox substitution value for a given byte value.
   @param v byte input value.
   @return inverse substitution for the given byte.
*/
static byte invSubstBox( byte v )
{
  // Inverse-direction replacement map for the sBox.
  static const byte irule[] =
  {  };
  
  // Return the (inverse) replaceent for v based on the map.
  return irule[ v ];
}

/**
 * This function computes the g function used in generating the subkeys from the original, 
 * 16-byte key. It takes a 4-byte input via the src parameter and returns a 4-byte result 
 * via the dest parameter. The value, r, gives the round number, between 1 and 10 (inclusive).
*/
void gFunction( byte dest[ WORD_SIZE ], byte const src[ WORD_SIZE ], int r )
{
  // Constant values used in each round of the g function.
  static const byte roundConstant[ ROUNDS + 1 ] = {
    0x00, // First element not used.
    0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36
  };

  // Assign destinations from source
  for ( int i = 1; i < WORD_SIZE; i++ ) {
    dest[i] = substBox( src[i + 1] );
  }
  dest[WORD_SIZE - 1] = substBox( src[0] );
  // Apply round constant
  dest[0] = fieldAdd( substBox( src[1] ), roundConstant[r] );
}

/*
This function fills in the subkey array with subkeys for each round of AES, computed from the given key.
*/
void generateSubkeys( byte subkey[ ROUNDS + 1 ][ BLOCK_SIZE ], byte const key[ BLOCK_SIZE ] )
{
  for ( int i = 0; i < BLOCK_SIZE; i++ ) {
    subkey[0][i] = key[i];
  }
  for ( int i = 1; i < ROUNDS + 1; i++ ) {
    
    //Separate previous subkey source
    byte src1[ WORD_SIZE ];
    byte src2[ WORD_SIZE ];
    byte src3[ WORD_SIZE ];
    byte src4[ WORD_SIZE ];
    byte dest[ WORD_SIZE ];
    for ( int k = 0; k < WORD_SIZE; k++ ) {
      src1[k] = subkey[i - 1][k];
      src2[k] = subkey[i - 1][k + WORD_SIZE];
      src3[k] = subkey[i - 1][k + WORD_SIZE + WORD_SIZE];
      src4[k] = subkey[i - 1][k + WORD_SIZE + WORD_SIZE + WORD_SIZE];
    }
    // Apply g function
    gFunction(dest, src4, i);

    // Make new subkey for 0-3
    byte sub1[ WORD_SIZE ];
    for (int k = 0; k < WORD_SIZE; k++ ) {
      sub1[k] = dest[k] ^ src1[k];
    }
    
    for ( int j = 0; j < WORD_SIZE; j++ ) {
      subkey[i][j] = sub1[j];
    }

    // Make new subkey for 4-7
    byte sub2[ WORD_SIZE ];
    for (int k = 0; k < WORD_SIZE; k++ ) {
      sub2[k] = sub1[k] ^ src2[k];
    }

    for ( int j = 0; j < WORD_SIZE; j++ ) {
      subkey[i][j + WORD_SIZE] = sub2[j];
    }

    // Make new subkey for 8-11
    byte sub3[ WORD_SIZE ];
    for (int k = 0; k < WORD_SIZE; k++ ) {
      sub3[k] = sub2[k] ^ src3[k];
    }

    for ( int j = 0; j < WORD_SIZE; j++ ) {
      subkey[i][j + WORD_SIZE + WORD_SIZE] = sub3[j];
    }

    // Make new subkey for 12-15
    byte sub4[ WORD_SIZE ];
    for (int k = 0; k < WORD_SIZE; k++ ) {
      sub4[k] = sub3[k] ^ src4[k];
    }
    for ( int j = 0; j < WORD_SIZE; j++ ) {
      subkey[i][j + WORD_SIZE + WORD_SIZE + WORD_SIZE] = sub4[j];
    }

  }
}

/**
 * This function performs the addSubkey operation, adding the given subkey (key) to the given data array.
*/
void addSubkey( byte data[ BLOCK_SIZE ], byte const key[ BLOCK_SIZE ] )
{
  for ( int i = 0; i < BLOCK_SIZE; i++ ) {
    data[i] = fieldAdd( data[i], key[i] );
  }
}

/**
 * This function rearranges a block of 16 data values from the one-dimensional arrangement to the 4 × 4 arrangement.
*/
void blockToSquare( byte square[ BLOCK_ROWS ][ BLOCK_COLS ], byte const data[ BLOCK_SIZE ] )
{
  int idx = 0;
  for ( int i = 0; i < BLOCK_ROWS; i++ ) {
    for ( int j = 0; j < BLOCK_COLS; j++ ) {
      square[j][i] = data[idx];
      idx++;
    }
  }
}

/**
 * This function rearranges a 4 × 4 arrangement of data values, returning them as a one-dimensional array of 16 values.
*/
void squareToBlock( byte data[ BLOCK_SIZE ], byte const square[ BLOCK_ROWS ][ BLOCK_COLS ] )
{
  int idx = 0;
  for ( int i = 0; i < BLOCK_ROWS; i++ ) {
    for ( int j = 0; j < BLOCK_COLS; j++ ) {
      data[idx] = square[j][i];
      idx++;
    }
  }
}

/**
 * This function performs the shiftRows operation on the given 4 × 4 square of values.
*/
void shiftRows( byte square[ BLOCK_ROWS ][ BLOCK_COLS ] )
{
  byte spare[ BLOCK_ROWS ][ BLOCK_COLS ];
  for ( int i = 0; i < BLOCK_ROWS; i++ ) {
    for ( int j = 0; j < BLOCK_COLS; j++ ) {
      spare[i][j] = square[i][j];
    }
  }
  int idx;
  for ( int i = 1; i < BLOCK_ROWS; i++ ) {
    for ( int j = 0; j < BLOCK_COLS; j++ ) {
      idx = ( j + i ) % WORD_SIZE;
      square[i][j] = spare[i][idx];
    }
  }
}

void unShiftRows( byte square[ BLOCK_ROWS ][ BLOCK_COLS ] )
{
  byte spare[ BLOCK_ROWS ][ BLOCK_COLS ];
  for ( int i = 0; i < BLOCK_ROWS; i++ ) {
    for ( int j = 0; j < BLOCK_COLS; j++ ) {
      spare[i][j] = square[i][j];
    }
  }
  int idx;
  for ( int i = 1; i < BLOCK_ROWS; i++ ) {
    for ( int j = 0; j < BLOCK_COLS; j++ ) {
      idx = ( j - i ) % WORD_SIZE;
      if ( idx < 0 ) {
        idx = WORD_SIZE + idx;
      }
      square[i][j] = spare[i][idx];
    }
  }
}

void mixColumns( byte square[ BLOCK_ROWS ][ BLOCK_COLS ] )
{
  // Matrix by which each column of square is multiplied.
  static const byte mixMatrix[ BLOCK_ROWS ][ BLOCK_COLS ] = {
    { 0x02, 0x03, 0x01, 0x01 },
    { 0x01, 0x02, 0x03, 0x01 },
    { 0x01, 0x01, 0x02, 0x03 },
    { 0x03, 0x01, 0x01, 0x02 }
  };

  byte spare[ BLOCK_ROWS ][ BLOCK_COLS ];
  for ( int i = 0; i < BLOCK_ROWS; i++ ) {
    for ( int j = 0; j < BLOCK_COLS; j++ ) {
      spare[i][j] = square[i][j];
    }
  }

  byte term[WORD_SIZE];
  for ( int i = 0; i < BLOCK_ROWS; i++ ) {
    for ( int j = 0; j < BLOCK_COLS; j++ ) {
      for ( int k = 0; k < WORD_SIZE; k++ ) {
        term[k] = fieldMul( mixMatrix[i][k], spare[k][j]);
      }
      byte a = fieldAdd( term[0], term[1] );
      byte b = fieldAdd( term[WORD_SIZE - 1 - 1], term[WORD_SIZE - 1] );
      square[i][j] = fieldAdd( a, b );
    }
  }
}

void unMixColumns( byte square[ BLOCK_ROWS ][ BLOCK_COLS ] )
{
  // Matrix by which each column of square is multiplied.
  static const byte invMixMatrix[ BLOCK_ROWS ][ BLOCK_COLS ] = {
    { 0x0E, 0x0B, 0x0D, 0x09 },
    { 0x09, 0x0E, 0x0B, 0x0D },
    { 0x0D, 0x09, 0x0E, 0x0B },
    { 0x0B, 0x0D, 0x09, 0x0E }
  };

  byte spare[ BLOCK_ROWS ][ BLOCK_COLS ];
  for ( int i = 0; i < BLOCK_ROWS; i++ ) {
    for ( int j = 0; j < BLOCK_COLS; j++ ) {
      spare[i][j] = square[i][j];
    }
  }
  
  byte term[WORD_SIZE];
  for ( int i = 0; i < BLOCK_ROWS; i++ ) {
    for ( int j = 0; j < BLOCK_COLS; j++ ) {
      for ( int k = 0; k < WORD_SIZE; k++ ) {
        term[k] = fieldMul( invMixMatrix[i][k], spare[k][j]);
      }
      byte a = fieldAdd( term[0], term[1] );
      byte b = fieldAdd( term[WORD_SIZE - 1 - 1], term[WORD_SIZE - 1] );
      square[i][j] = fieldAdd( a, b );
    }
  }
}

/**
 * This function encrypts a 16-byte block of data using the given key. It generates
 * the 11 subkeys from key, adds the first subkey, then performs the 10 rounds of
 * operations needed to encrypt the block.
*/
void encryptBlock( byte data[ BLOCK_SIZE ], byte key[ BLOCK_SIZE ] )
{
  // Generate subkeys
  byte subkey[ ROUNDS + 1 ][ BLOCK_SIZE ];
  generateSubkeys( subkey, key );
  // Add first subkey (the original key)
  addSubkey( data, key );
  // Go through 10 rounds
  for ( int i = 1; i < ROUNDS + 1; i++ ) {
    // Apply substitution function
    for (int j = 0; j < BLOCK_SIZE; j++ ) {
      data[j] = substBox( data[j] );
    }
    // Convert data to block form
    byte square[ BLOCK_ROWS ][ BLOCK_COLS ];
    blockToSquare( square, data );
    // Disturb data
    shiftRows( square );
    // Disturb data, omit if last round
    if ( i != ROUNDS ) {
      mixColumns( square );
    }
    // Convert data back to line form
    squareToBlock( data, square );
    // Apply new subkey
    addSubkey( data, subkey[i]);
  }
}

void decryptBlock( byte data[ BLOCK_SIZE ], byte key[ BLOCK_SIZE ] )
{
  // Generate subkeys
  byte subkey[ ROUNDS + 1 ][ BLOCK_SIZE ];
  generateSubkeys( subkey, key );
  for ( int i = ROUNDS; i > 0; i-- ) {
    addSubkey(data, subkey[i]);
    byte square[ BLOCK_ROWS ][ BLOCK_COLS ];
    blockToSquare( square, data );
    if ( i != ROUNDS ) {
      unMixColumns( square );
    }
    unShiftRows( square );
    squareToBlock( data, square );
    for (int j = 0; j < BLOCK_SIZE; j++ ) {
      data[j] = invSubstBox( data[j] );
    }
  }
  addSubkey( data, key );
  

}
